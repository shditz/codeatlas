...
export function registerAnalyzeCommand(program: Command): void {
  program
    .command('analyze')
    .description('Run deep architectural graph analytics (dead code, circular dependencies, complexity hotspots)')
    ...
    .action(async (options: { cycles?: boolean; deadCode?: boolean; hotspots?: boolean; json?: boolean }) => {
      ...
      if (options.json) {
        if (options.cycles) {
          console.log(JSON.stringify({ cycles: report.cycles, count: report.cycles.length }, null, 2));
        } else if (options.deadCode) {
          console.log(JSON.stringify({ deadCode: report.deadCode, count: report.deadCode.length }, null, 2));
        } else if (options.hotspots) {
          console.log(JSON.stringify({ hotspots: report.hotspots, instabilities: report.instabilities }, null, 2));
        } else {
      ...
      if (showAll || options.cycles) {
        ...
        if (report.cycles.length === 0) {
          ...
          console.log(chalk.red.bold(`  ✖ Found ${report.cycles.length} circular dependency cycle(s):`));
      ...
      if (showAll || options.deadCode) {
        ...
        if (report.deadCode.length === 0) {
          ...
          console.log(chalk.yellow(`  ⚠ Found ${report.deadCode.length} candidate file(s) with 0 incoming dependencies:`));
