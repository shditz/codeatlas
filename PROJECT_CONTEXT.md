...
| **`@codeatlas-ai/mcp`**           | Model Context Protocol JSON-RPC 2.0 implementation with 12 tools, 3 resources, 3 prompts.                          | [`packages/mcp/src/index.ts`](file:///c:/Users/DELL/Downloads/CodeAtlas/packages/mcp/src/index.ts)                     |
...
### 3.10 Model Context Protocol (MCP) Server (`@codeatlas-ai/mcp`, `apps/mcp-server`)

- Provides standard JSON-RPC 2.0 stdio transport compatible with Claude Desktop, Cursor, Antigravity, Windsurf, and VS Code.
- Exposes **12 Tools**, **3 Resources**, and **3 Prompts**.

---

## 4. Complete CLI Commands Reference (16 Commands)
...
| **`atlas index`**          | Parses ASTs, extracts symbols/imports, and builds SQLite index and dependency graph. | `--force`, `--verbose`                                                                       |
| **`atlas map`**            | Displays visual tree map of codebase structure with exported symbols.                | `--depth <n>`, `--json`, `--module <name>`                                                   |
| **`atlas search <query>`** | Performs full-text & symbol search with BM25 ranking.                                | `--limit <n>`, `--type <file\|symbol>`, `--json`                                             |
| **`atlas context <task>`** | Packs token-budgeted, ranked context for a coding prompt.                            | `--max-tokens <n>`, `--mode <full\|signature\|summary\|digest>`, `--output <file>`, `--json` |
...
| **`atlas doctor`**         | Diagnoses database health, missing indexes, and repository readiness.                | `--fix`                                                                                      |
| **`atlas clean`**          | Cleans `.atlas/` database, cached snapshots, and temporary files.                    | `--all`                                                                                      |
| **`atlas mcp`**            | Starts the Model Context Protocol (MCP) stdio server.                                | _(stdio transport)_                                                                          |
| **`atlas watch`**          | Runs live watcher for real-time incremental re-indexing.                             | `--debounce <ms>`                                                                            |
| **`atlas diff`**           | Analyzes git branch diff and affected downstream dependent modules.                  | `--base <branch>`, `--json`                                                                  |
...
## 5. Complete MCP Tools Reference (12 Tools)

The CodeAtlas MCP Server exposes 12 tools for AI agents:

```json
1. atlas_scan           -> Scan repository architecture, languages, and workspaces
2. atlas_index          -> Trigger full or incremental AST indexing
3. atlas_search         -> FTS5 BM25 search across files, symbols, and signatures
4. atlas_get_context    -> Retrieve ranked, token-budgeted context pack for a task
5. atlas_graph_query    -> Execute Cypher or Natural Language graph queries
6. atlas_pr_diff        -> Analyze Git PR diff with architectural impact analysis
7. atlas_compress       -> Extract AST signature skeleton to save token budget
8. atlas_get_map        -> Retrieve hierarchical directory structure and exports
9. atlas_get_rules      -> Discover and audit active AI agent rules & conflicts
10. atlas_doctor        -> Check database health and index statistics
11. atlas_analyze       -> Detect circular dependencies, dead code, and hotspots
12. atlas_sql_query     -> Execute read-only SQL queries directly on .atlas/atlas.db
```
