...
import { DependencyGraph, GraphQueryEngine } from '../src/index.js';
import type { GraphNodeItem, GraphEdgeItem } from '../src/index.js';

describe('Graph Query Engine', () => {
  it('should parse and execute simple node match query', () => {
    const graph = new DependencyGraph();
    const nodes: GraphNodeItem[] = [
      { id: 'src/auth.ts', label: 'File', properties: { name: 'auth.ts', path: 'src/auth.ts', lines: 120 } },
      { id: 'src/db.ts', label: 'File', properties: { name: 'db.ts', path: 'src/db.ts', lines: 300 } },
    ];
  ...
  it('should execute edge traversal query with type and properties', () => {
    const graph = new DependencyGraph();
    const nodes: GraphNodeItem[] = [
      { id: 'func:login', label: 'Symbol', properties: { name: 'login', kind: 'function', file: 'auth.ts' } },
      { id: 'func:queryUser', label: 'Symbol', properties: { name: 'queryUser', kind: 'function', file: 'db.ts' } },
      { id: 'func:renderUI', label: 'Symbol', properties: { name: 'renderUI', kind: 'function', file: 'ui.ts' } },
    ];
    ...
    const result = engine.execute('MATCH (s:Symbol {kind: "function"})-[:CALLS]->(t:Symbol) WHERE s.name = "login" RETURN s.name, t.name');
  ...
  it('should filter with CONTAINS and numerical operators', () => {
    ...
    const engine = new GraphQueryEngine(graph, nodes);
    const result = engine.execute('MATCH (f:File) WHERE f.path CONTAINS "packages" AND f.lines > 100 RETURN f.path');
