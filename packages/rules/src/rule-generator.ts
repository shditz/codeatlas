...
`);
  ...
  static readonly START_MARKER = '<!-- CODEATLAS:START_ARCHITECTURE_MAP -->';
  static readonly END_MARKER = '<!-- CODEATLAS:END_ARCHITECTURE_MAP -->';

  static syncArchitectureBlock(existingContent: string, newBlock: string): string {
    const startIdx = existingContent.indexOf(RuleGenerator.START_MARKER);
    const endIdx = existingContent.indexOf(RuleGenerator.END_MARKER);

    const fullBlock = `${RuleGenerator.START_MARKER}\n${newBlock.trim()}\n${RuleGenerator.END_MARKER}`;

    if (startIdx !== -1 && endIdx !== -1 && endIdx >= startIdx) {
      const before = existingContent.substring(0, startIdx);
      const after = existingContent.substring(endIdx + RuleGenerator.END_MARKER.length);
  ...
  generateArchitectureBlock(meta: {
    projectName: string;
    totalFiles?: number;
    totalSymbols?: number;
    totalDependencies?: number;
    keyModules?: Array<{ name: string; path: string; description?: string }>;
    boundedContexts?: Array<{ name: string; files: string[] }>;
    circularDependencyCount?: number;
  }): string {
    let content = `### 🗺️ CodeAtlas Live Architecture & Dependency Blueprint
> Auto-generated by CodeAtlas Engine. Keep this section intact for AI Agent context synchronization.

- **Repository**: \`${meta.projectName}\`
- **Indexed Scope**: ${meta.totalFiles ?? 0} files | ${meta.totalSymbols ?? 0} symbols | ${meta.totalDependencies ?? 0} dependencies
${
  meta.circularDependencyCount !== undefined && meta.circularDependencyCount > 0
    ? `- ⚠️ **Active Circular Dependencies**: ${meta.circularDependencyCount} (Must be refactored)`
    : '- ✅ **Architecture Health**: Clean DAG (0 circular dependencies)'
}
`;

    if (meta.keyModules && meta.keyModules.length > 0) {
      content += `\n#### 📦 Key Architectural Modules & Boundaries:\n`;
      for (const mod of meta.keyModules) {
        content += `- \`${mod.path}\`: ${mod.description || mod.name}\n`;
      }
    }

    if (meta.boundedContexts && meta.boundedContexts.length > 0) {
      content += `\n#### 🛡️ Bounded Contexts & Access Rules:\n`;
      for (const ctx of meta.boundedContexts) {
        content += `- **${ctx.name}**: ${ctx.files.slice(0, 5).join(', ')}${
          ctx.files.length > 5 ? ` (+${ctx.files.length - 5} more)` : ''
        }\n`;
      }
    }

    content += `\n#### ⚡ AI Agent Directives:
1. Always check incoming dependent files (Blast Radius) before modifying core exported interfaces.
2. Depend only across designated workspace/layer boundaries and avoid introducing new cyclic imports.
3. Use \`atlas context "<prompt>"\` for targeted AST retrieval.`;
