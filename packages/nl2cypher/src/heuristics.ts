export class HeuristicQueryGenerator {
  generate(nlPrompt: string): string | null {
    ...
    const callsMatch = raw.match(/(?:who calls|siapa yang memanggil|fungsi yang memanggil|calls to)\s+['"]?([a-zA-Z0-9_$]+)['"]?/i);
    ...
    const calleeMatch = raw.match(/(?:what does|fungsi yang dipanggil oleh)\s+['"]?([a-zA-Z0-9_$]+)['"]?\s*(?:call)?/i);
    ...
    const importMatch = raw.match(/(?:files importing|file yang (?:meng)?import|which files import|who imports)\s+['"]?([a-zA-Z0-9._\-/]+)['"]?/i);
    ...
    const fileImportsMatch = raw.match(/(?:what does|apa yang di(?:-|\s*)?import oleh)\s+['"]?([a-zA-Z0-9._\-/]+)['"]?\s*(?:import)?/i);
    ...
    const funcMatch = raw.match(/(?:find function|cari fungsi|search function)\s+['"]?([a-zA-Z0-9_$]+)['"]?/i);
    if (funcMatch && funcMatch[1]) {
      return `MATCH (s:Symbol) WHERE s.name CONTAINS '${funcMatch[1]}' RETURN s`;
    }

    // 7. Find class / Cari class X
    const classMatch = raw.match(/(?:find class|cari class|cari kelas)\s+['"]?([a-zA-Z0-9_$]+)['"]?/i);
    if (classMatch && classMatch[1]) {
      return `MATCH (s:Symbol) WHERE s.name CONTAINS '${classMatch[1]}' RETURN s`;
    }

    // 8. Find file / Cari file X
    const fileMatch = raw.match(/(?:find file|cari file|search file)\s+['"]?([a-zA-Z0-9._\-/]+)['"]?/i);
